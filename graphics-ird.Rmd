...
ta_centres <- select(TA2013, TA = TA2013_NAM, WGS84Longitude, WGS84Latitude) |>
  as_tibble()
...
the_data <- combined %>%
    ...
    left_join(ta_centres) %>%
    mutate(TA = gsub(" District", "", TA),
           TA = gsub(" City", "", TA),
           outlier = ifelse(CAGR10 > 0.05 | CAGR10 < -0.02, TA, "")) |>
  rename(long.centre = WGS84Longitude, lat.centre = WGS84Latitude)
stopifnot(sum(is.na(the_data$lat.centre)) == 0)
